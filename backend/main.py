...
def record_audit(message: str, intent: str, tool: str, started: float) -> None:
    AUDIT_LOG.insert(0, {"user_id": "officer01", "query_text": message, "intent": intent, "tool_used": tool, "execution_status": "success", "response_time_ms": round((time.perf_counter() - started) * 1000), "created_at": datetime.now().isoformat(timespec="seconds")})
    del AUDIT_LOG[50:]
...
def login(payload: LoginRequest) -> dict[str, Any]:
    if payload.username == "officer01" and payload.password == "demo123":
        return {"authenticated": True, "user": {"username": payload.username, "role": "Simulated Polling Officer"}}
    raise HTTPException(401, "Use the demo credentials officer01 / demo123")
...
@app.get("/api/voters")
def list_voters(search: str = "", status: str = "", limit: int = Query(25, ge=1, le=100)) -> dict[str, Any]:
...
def get_voter(voter_id: str) -> dict[str, Any]:
...
def get_station_statistics(station_id: str) -> dict[str, Any]:
    return station_stats(station_id.upper())


@app.get("/api/analytics/hourly")
def hourly() -> list[dict[str, int]]:
...
def anomalies(limit: int = Query(50, ge=1, le=200)) -> dict[str, Any]:
...
def chat_answer(message: str) -> dict[str, Any]:
    ...
    if voter_match:
        ...
        record_audit(text, result["intent"], result["tool_used"], started)
        return result
...
def chat(payload: ChatRequest) -> dict[str, Any]:
    return chat_answer(payload.message)
